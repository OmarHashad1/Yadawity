...
function completeUserRegistration($db, $data, $isActive = true) {
    try {
        ...
        // Use provided profile picture or let database default handle it
        ...
        if ($data['user_type'] === 'artist') {
            ...
            if ($profilePic) {
                ...
                // Let database default handle profile_picture
                $stmt = $db->prepare("INSERT INTO users (email, password, first_name, last_name, phone, user_type, bio, art_specialty, years_of_experience, national_id, is_active, is_verified, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, 0, NOW())");
            ...
            if ($profilePic) {
                ...
                // Let database default handle profile_picture
                $stmt = $db->prepare("INSERT INTO users (email, password, first_name, last_name, phone, user_type, is_active, is_verified, created_at) VALUES (?, ?, ?, ?, ?, ?, ?, 0, NOW())");
                
                if (!$stmt) {
                    throw new Exception("Failed to prepare buyer registration query: " . $db->error);
                }
                
                $stmt->bind_param("ssssssi", $data['email'], $data['password'], $data['first_name'], $data['last_name'], $data['phone'], $data['user_type'], $isActiveValue);
            }
        }
...
function processProfileUpload($db) {
    try {
        ...
        if (!empty($_FILES['profile_picture']['name'])) {
            ...
            // No profile picture uploaded, database default will be used
            // No action needed as the default is already set in the users table
        }
    ...
    try {
        ...
        if (empty($errors)) {
            ...
            if ($result['success']) {
                ...
                // Upload profile picture if provided, otherwise set default placeholder
                if (!empty($_FILES['profile_picture']['name'])) {
                    $uploadResult = uploadProfilePicture(
                        $_FILES['profile_picture'], 
                        $userId, 
